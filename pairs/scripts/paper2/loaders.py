...
def join_backtests_by_id(analysis: pd.DataFrame, ids: List[int] = None, drop_garbage=True):
    ...
    for id in tqdm(ids, desc="Loading backtests by id"):
        ...
        if drop_garbage:
            aggregated = aggregated.drop(labels=['1Weights', '2Weights', '1Price', '2Price', 'Spread', 'normLogReturns'], axis=1)
...
def process_experiment2(
    ...
    if new_txcosts is not None:
        new_rows = calculate_new_experiments_txcost(
            analysis, new_txcosts, add_backtests=True, original_only=True
        )
        analysis = analysis.append(new_rows)
        analysis = analysis.reset_index()
