...
color = 'tab:red'
line2 = model['cumProfit'].fillna(method='ffill').fillna(method='bfill').plot(color=color, ax=ax2, label='Value of investment', linestyle='-.')
...
plt.style.use('default')
df = pd.read_pickle('C:\\Users\\kawga\\Documents\\IES\Bach\\code\\Pairs-trading\\tables\\retstats.pkl')
fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, sharex=True)
plt.xlim(-0.2,0.2)
plt.sca(ax1)
plt.title('Daily')
ax1.set_ylabel('Distance')
rv=scipy.stats.norm(loc=df['Daily', 'Distance']['Mean'], scale=df['Daily', 'Distance']['Std'])
...
plt.sca(ax2)
plt.title('Hourly')
rv=scipy.stats.norm(loc=df['Hourly', 'Distance']['Mean'], scale=df['Hourly', 'Distance']['Std'])
plt.ylim(0,55)
sns.kdeplot(rethd, linewidth=1)
sns.kdeplot(rv.rvs(size=100000), linewidth=1, label='Normal', linestyle = '--', color='tab:red')

plt.sca(ax3)
rv=scipy.stats.norm(loc=df['Daily', 'Cointegration']['Mean'], scale=df['Daily', 'Cointegration']['Std'])
ax3.set_ylabel('Cointegration')
sns.kdeplot(retdc, linewidth=1)
sns.kdeplot(rv.rvs(size=100000), linewidth=1, label='Normal', linestyle = '--', color='tab:red')

plt.sca(ax4)
rv=scipy.stats.norm(loc=df['Hourly', 'Cointegration']['Mean'], scale=df['Hourly', 'Cointegration']['Std'])
plt.ylim(0,55)
...
fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, sharex=True)
plt.sca(ax1)
scipy.stats.probplot(retdd.dropna().values.astype('float32'), fit=True, plot=ax1, dist='norm')
plt.title(r'$\bf{Daily}$')
plt.xlabel("")
plt.ylabel(r'$\bf{Distance}$' +'\n' +r'$\mathrm{Ordered \: values}$', fontsize=11)
plt.sca(ax2)
scipy.stats.probplot(rethd.dropna().values.astype('float32'), fit=True, plot=ax2, dist='norm')
plt.title(r'$\bf{Hourly}$')
plt.ylabel("")
plt.xlabel("")
plt.sca(ax3)
scipy.stats.probplot(retdc.dropna().values.astype('float32'), fit=True, plot=ax3, dist='norm')
plt.title("")
plt.ylabel(r'$\bf{Cointegration}$' +'\n Ordered values')
plt.sca(ax4)
scipy.stats.probplot(rethc.dropna().values.astype('float32'), fit=True, plot=ax4, dist='norm')
plt.title("")
plt.ylabel("")
...
#%%
meanplot = hdisttable[[('Distance', 'Returns distribution', 'Mean'),('Cointegration', 'Returns distribution', 'Mean')]]
