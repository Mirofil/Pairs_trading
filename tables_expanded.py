...
results = pd.DataFrame(index=['\# of unit roots (n=23)'], columns = ['neither' ,'$\mu$', '$\mu + t$', '$\mu + t + t^2$'])
...
multitable.loc[("\% of identical pairs", 'Vol. cutoff'), 'hourly']=find_same(r_dist_hourly, r_coint_hourly)
multitable.loc[("\% of identical pairs", 'Vol. cutoff'), 'daily']=find_same(r_dist_daily, r_coint_daily)
...
#%%
index = ['Mean', 'Std', 'Max', 'Min', 'Jarque-Bera p-value', 
    'Skewness', 'Kurtosis', 'Positive', 't-stat']
...
'Avg length of position', 'Pct of winning trades', 'Max drawdown', 'Nominated pairs', 'Traded pairs']
...
feasible = ['Monthly profit', 'Annual profit' ,'Total profit',  'Annualized Sharpe','Trading period Sharpe', 'Number of trades', 'Roundtrip trades', 
'Avg length of position', 'Pct of winning trades', 'Max drawdown', 'Nominated pairs', 'Traded pairs']
agg = aggregate([ddd, ddc, dhd, dhc,dtd, dtc], feasible, trades_nonzero=True, returns_nonzero=True, freqs=[60,60,10,10,3,3])
agg=standardize_results(agg, poslen = [1,1,1/24,1/24,1/288, 1/288], numtrades=[1/2, 1/2, 3,3, 10,10])
agg= beautify(agg)
...
aggnl = aggregate([dddnl, ddcnl, dhdnl, dhcnl], feasible, trades_nonzero=True, returns_nonzero=True, freqs=[60,60,10,10,3,3])
...
'Avg length of position', 'Pct of winning trades', 'Max drawdown']
#careful about ordering of commited/employed because it changes xxxnl inplace
aggnl_commited = aggregate([dddnl, ddcnl, dhdnl, dhcnl, dtdnl, dtcnl], feasible, trades_nonzero=True,freqs=[60,60,10,10,3,3])
aggnl = aggregate([dddnl, ddcnl, dhdnl, dhcnl, dtdnl, dtcnl], feasible, trades_nonzero=True, returns_nonzero=True, freqs=[60,60,10,10,3,3])
aggnl.loc['Monthly profit (committed)'] = aggnl_commited.loc['Monthly profit']
aggnl=standardize_results(aggnl, poslen = [1,1,1/24,1/24,1/288, 1/288], numtrades=[1/2, 1/2, 3,3, 10,10])
#standardize results has to be first because it renames some rows
aggnl = aggnl.loc[['Monthly profit', 'Monthly profit (committed)','Annualized Sharpe', 'Monthly number of trades',
'Roundtrip trades', 'Length of position (days)', 'Pct of winning trades', 'Max drawdown']]
aggnl=beautify(aggnl)
...
feasible = ['Monthly profit', 'Annual profit' ,'Total profit',  'Annualized Sharpe','Trading period Sharpe', 'Number of trades', 'Roundtrip trades', 
'Avg length of position', 'Pct of winning trades', 'Max drawdown']
aggtx = aggregate([dddtx, ddctx, dhdtx, dhctx, dtdtx, dtctx], feasible, trades_nonzero=True, returns_nonzero=True, freqs=[60,60,10,10,3,3])
aggtx=standardize_results(aggtx, poslen = [1,1,1/24,1/24,1/288, 1/288], numtrades=[1/2, 1/2, 3,3, 10,10])
aggtx=beautify(aggtx)
