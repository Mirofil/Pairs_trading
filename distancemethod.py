...
def distance(df, num=5):
    ...
    for pair in pairs:
        newdf.loc[pair, 'logReturns']=(np.log(newdf.loc[pair, 'Close'])-np.log(newdf.loc[pair,'Close'].shift(1))).values
        newdf.loc[pair, 'normReturns'] = ((newdf.loc[pair,'logReturns']-newdf.loc[pair,'logReturns'].mean())/newdf.loc[pair, 'logReturns'].std()).values
    ...
    for i in range(distances.shape[0]):
        for j in range(i, distances.shape[1]):
            distances[i,j]=np.sum(np.power(
                newdf.loc[pairs[i], 'normPrice']-newdf.loc[pairs[j], 'normPrice'], 2))
    ...
    top_indexes = np.unravel_index(sorted_array[nonzero_index:nonzero_index+num], distances.shape)
...
def distance_spread(df, viable_pairs, timeframe, betas=1):
    ...
    for pair, coefs in zip(viable_pairs, betas):
        ...
        newdf['Spread'] = (-coefs[1]*df.loc[pair[0], 'Price']+df.loc[pair[1], 'Price']).values
...
def adjust_weights(olddf, copy=True):
    ...
    price_diff = (1+(df.groupby(level=0)['1Price'].shift(0)-df.groupby(level=0)['1Price'].shift(1))).groupby(level=0).cumprod()
    df['1Weights']=df['1Weights'].groupby(level=0).shift(1)*price_diff.groupby(level=0).shift(0)
    price_diff2 = (1+(df.groupby(level=0)['2Price'].shift(0)-df.groupby(level=0)['2Price'].shift(1))).groupby(level=0).cumprod()
    df['2Weights']=df['2Weights'].groupby(level=0).shift(1)*price_diff2.groupby(level=0).shift(0)
    return df
...
def distance_profit(df, beta=1.5):
    ...
    pure_1return = (df.groupby(level=0)['1Price'].shift(0)-df.groupby(level=0)['1Price'].shift(1))
    ...
    pure_2return = (df.groupby(level=0)['2Price'].shift(0)-df.groupby(level=0)['2Price'].shift(1))
