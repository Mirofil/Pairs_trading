...
df['Daily', 'Distance']=summarize(retdd, index).values
...
aggnl = aggregate([dddnl, ddcnl, dhdnl, dhcnl], feasible, trades_nonzero=True, returns_nonzero=True)
...
#%%
#BIG STOPLOSS TABLE
# resdd= stoploss_results(methods=['dist'], freqs=['daily'])
# resdc= stoploss_results(methods=['coint'], freqs=['daily'])
# reshd=stoploss_results(methods=['dist'], freqs=['hourly'])
